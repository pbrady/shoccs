...
- [x] **45.7b.1** (review follow-up to 45.7b) Fix `docs/handoff/MASTER.md` committed with broken internal links and stale content. Commit 1882cc3 added `docs/handoff/MASTER.md` to the repo, but the six sibling files it links to (`framework_architecture.md`, `completed_plans.md`, `scientific_findings.md`, `next_steps.md`, `operating_conventions.md`, `known_limitations.md`) exist only in the working tree and are untracked — confirmed via `git ls-tree HEAD docs/handoff/` returning just `MASTER.md` and `git status docs/handoff/` listing all six as `??`. On a fresh clone, ~15 Markdown links inside MASTER.md resolve to missing files (the numbered "Read in this order" section, the "What to do if the user asks about a specific topic" routing table, and the footer links). Separately, MASTER.md's own content is dated 2026-04-15 (it was drafted during plan-45 scoping and landed 2026-04-24 during plan-45 wind-down) and now contradicts repo reality in several places: line 7 / 15 / 25 say "plans 40–44"; line 17 calls plan 45 "actively being scoped"; line 61 lists **11** `python -m sweeps` subcommands but `sweeps/__main__.py` now registers **12** (the `pareto` subcommand was added in 45.3b between `gv-stability-pareto` and `brady2d`); line 65 states "Plan 45 doesn't exist yet"; line 83 tells future agents to "propose the scope" for plan 45. Fix both issues in a single commit:
  1. Track the six sibling files. They were dropped by the handoff author and contain real architectural documentation referenced by MASTER.md; committing them with a note that they are a 2026-04-15 snapshot is the minimum to make MASTER.md's links resolve. Use `git add docs/handoff/{framework_architecture,completed_plans,scientific_findings,next_steps,operating_conventions,known_limitations}.md`. If any of the sibling files are deemed unsuitable to track as-is, delete the corresponding link from MASTER.md instead of committing a broken tree — do NOT leave the mixed state the commit currently produces.
  2. Refresh MASTER.md's plan-45-era facts: change "plans 40–44" → "plans 40–45" in the "What this codebase is" paragraph (line 7), the "Recent state" bullet (line 15), and the `completed_plans.md` description (line 25); update "Recent state (2026-04-15)" → "Recent state (2026-04-24)" and rewrite its plan-45 bullet to reflect substantial completion (front JSON calibration done, C++ validation wiring done, regression test landed; 45.7c–e remaining); bump the subcommand count from 11 to 12 and insert `pareto` into the registered-subcommands list between `gv-stability-pareto` and `brady2d` (line 61); strike "Plan 45 doesn't exist yet" from the Plans bullet (line 65) and replace with "Plan 45 is in its final docs/skills phase (45.7c–e remaining); plans 46–47 queued per `next_steps.md`"; rewrite the "continue" routing step 3 (line 83) to say plan 45 is landing its docs/skills tail and point the agent at `next_steps.md` for plans 46+ instead. Keep the "Session-context distillation" section untouched — it is an accurate snapshot of the design-session findings and doesn't rot.
  - File: `docs/handoff/MASTER.md`, `docs/handoff/framework_architecture.md`, `docs/handoff/completed_plans.md`, `docs/handoff/scientific_findings.md`, `docs/handoff/next_steps.md`, `docs/handoff/operating_conventions.md`, `docs/handoff/known_limitations.md`
  - Test: `cd /workspace && git ls-tree HEAD docs/handoff/ | wc -l` returns `7` (was `1` after 1882cc3); every bare filename referenced from `MASTER.md` resolves to a tracked path:
    ```bash
    for f in framework_architecture completed_plans scientific_findings next_steps operating_conventions known_limitations; do
      git ls-tree HEAD "docs/handoff/$f.md" | grep -q "$f.md" || { echo "MISSING: $f.md"; exit 1; }
    done
    ```
    And the stale-fact greps return nothing: `grep -E "11 subcommands|Plan 45 doesn't exist|actively being scoped" docs/handoff/MASTER.md || echo OK`.
  - **Implementation note:** All six sibling files staged via `git add docs/handoff/{framework_architecture,completed_plans,scientific_findings,next_steps,operating_conventions,known_limitations}.md` so every link inside MASTER.md now resolves. MASTER.md refreshed per the plan enumeration: "plans 40–44" → "plans 40–45" on lines 7/15/25; "Recent state (2026-04-15)" → "(2026-04-24)" with a rewritten plan-45 bullet reflecting substantial completion (CLI + calibration JSONs + regression test + pareto_reference.md live; 45.7c–e remaining); subcommand count 11 → 12 with `pareto` inserted between `gv-stability-pareto` and `brady2d`; "Plan 45 doesn't exist yet" replaced with a pointer to `plans/45-pareto-optimization.md`; "continue" routing step 3 rewritten to describe the 45.7c–e tail. Additionally refreshed two internally-inconsistent adjacencies not strictly enumerated but required for coherence: the `next_steps.md` file-guide description (line 29) noting plan 45 has since landed, and the one-line summary footer (line 107) bumping to "six plans deep" with pymoo/NSGA-II surfaced. "Session-context distillation" left untouched as specified. Stale-fact grep returns `OK`; `git ls-tree HEAD docs/handoff/` will return 7 entries after commit.

- [x] **45.7b.2** (review follow-up to 45.7b) Fix the Brady-Livescu 1D Euler plan-number deferral in `scripts/stencil_gen/docs/optimization_reference.md:338`. The 45.7b commit correctly shifted the "Multi-fidelity Bayesian optimization" bullet from "Deferred to plan 45" → "Deferred to plan 46" to track `docs/handoff/next_steps.md`, but the adjacent "Brady-Livescu 1D Euler reproduction" bullet at line 336–338 still reads "Deferred to plan 46", and now two bullets in the same "Known limitations" list both claim plan 46 — a plan cannot be both Bayesian multi-fidelity AND a 1D Euler reimplementation. `docs/handoff/next_steps.md` is unambiguous: line 73 titles plan 46 as "Multi-Fidelity Bayesian Optimization"; line 90 titles plan 47 as "Brady-Livescu 1D Euler Reproduction". The fix is a one-token change: `Deferred to plan 46.` → `Deferred to plan 47.` on the Brady-Livescu bullet. Re-check after the edit that `grep -c "Deferred to plan 46" scripts/stencil_gen/docs/optimization_reference.md` returns exactly `1`.
  ...
  - **Implementation note:** One-token edit on `optimization_reference.md:338` (`Deferred to plan 46.` → `Deferred to plan 47.`). Verified: `grep -n "Deferred to plan"` returns exactly one "plan 46" (line 335, Emukit/Bayesian) and one "plan 47" (line 338, Brady-Livescu 1D Euler). Aligns with `docs/handoff/next_steps.md` plan numbering (plan 46 = Bayesian multi-fidelity; plan 47 = Brady-Livescu reproduction).

- [x] **45.7b.3** (review follow-up to 45.7b.1) Reconcile the remaining MASTER.md-vs-sibling contradictions introduced by commit `b4e983c`. The 45.7b.1 implementation tracked the six siblings and refreshed MASTER.md to claim "plans 40–45", but the siblings themselves still carry 2026-04-15 framing that MASTER.md now falsely contradicts. The plan text for 45.7b.1 explicitly warned "do NOT leave the mixed state the commit currently produces"; the commit landed a new mixed state. Three concrete contradictions remain:
  ...
  **Option B (more work, cleaner long-term)**: add a `### Plan 45 — Multi-Objective Pareto Optimization (NSGA-II)` section to `completed_plans.md` mirroring the plans 40–44 template (Deliverables table, Public API, Corrections); strike the two resolved items from `known_limitations.md` (replacing with a one-line "Fixed in plan 45" stub pointer); delete the "Plan 45" section from `next_steps.md` and renumber the "Where the next agent should start" routing. This is substantially more content work (~100 lines of new summary), justified only if the user wants the handoff folder to be a single-source-of-truth rather than an append-only snapshot archive.
  ...
  - **Implementation note:** Took Option A (the recommended minimal path). Added a 3-line `> **Snapshot status (2026-04-15).**` banner immediately after the H1 in each of `completed_plans.md`, `known_limitations.md`, and `next_steps.md`. The banner explicitly names the three forms of staleness a reader might encounter (plan 45 "queued", `gate_layer` auto-infer unimplemented, multi-objective optimization "not yet available") so an agent can identify them in context without having to cross-reference `plans/45-pareto-optimization.md` for every stale claim. Fixed MASTER.md line 25 from `summary of what plans 40–45 delivered` → `summary of what plans 40–44 delivered (plan 45 landed after this snapshot; see `plans/45-pareto-optimization.md`)`, exactly mirroring the caveat pattern that the 45.7b.1 commit already used on the `next_steps.md` description (line 29). Verification: `grep -c "plans 40–45 delivered" docs/handoff/MASTER.md` → `0`; `grep -c "Snapshot status" docs/handoff/*.md` → 3 files with 1 hit each (completed_plans.md, known_limitations.md, next_steps.md); the two stale `gate_layer`/plan-45 mentions in `known_limitations.md` (lines 13 and the deferred-multi-objective section) now sit below a banner, satisfying the Option A criterion. Option B (a full plan-45 section in `completed_plans.md` + struck-through known_limitations entries + `next_steps.md` renumbering) was skipped as ~100 lines of summary-writing disproportionate to the value given that `plans/45-pareto-optimization.md` itself is the canonical record.
