...
- [x] **7.2** Migrate `selections.hpp`
  ...
  - [ ] **7.2f** Fix `FView::end()` backward-traversal bug and add multi-line test (review finding from 7.2e):
...
12. **7.2e** (compile-instantiation test for `selections.hpp`) is done. **7.2f** (FView end() bug fix + multi-line test) should be done next — it fixes a correctness bug in 7.2c's FView migration where `end()` stores `l = lines.size()` (out of bounds), breaking backward traversal on multi-line FViews.
