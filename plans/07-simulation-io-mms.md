...
- [x] **7.2** Migrate `selections.hpp`
  ...
  - [x] **7.2e** Add compile-instantiation test for `selections.hpp` (unblocks confidence in 7.2a–7.2d): **DONE** — created `src/mesh/selections.t.cpp` with static_asserts verifying `YPlaneView<span<int>>` and `FView<span<int>>` satisfy `std::ranges::random_access_range` and `std::ranges::sized_range`, plus runtime tests exercising `begin()`/`end()`/`size()` on both view types. Also added stub concept/trait definitions (`DomainSelection`, `Selection`, `is_domain_selection_v`, `is_Rx_selection_v`, `is_Ry_selection_v`, `is_Rz_selection_v`) to `selections.hpp` — these were used by utility functions (xmin, xmax, ..., location, F) but undefined in the codebase (lost during prior refactoring). Added `add_unit_test(selections "mesh" shoccs-mesh)` to `src/mesh/CMakeLists.txt`. Test: `ctest -R t-selections` passes (8 assertions, 2 test cases).
  ...
  - [x] **7.2f** Fix `FView::end()` backward-traversal bug and add multi-line test (review finding from 7.2e): **DONE** — changed `end()` to set `l = lines.size() - 1` instead of `l = lines.size()` (one-past-end), matching the state that `operator++` produces at the end. Added multi-line FView test case with 2 lines (4+5=9 elements) verifying: forward iteration across line boundary, `operator--` from `end()` producing last element, `operator--` walking backward across line boundary, `operator+=(-n)` from `end()` crossing line boundaries. Test: `ctest -R t-selections` passes (24 assertions, 3 test cases).
    - **Bug:** `FView::end()` sets `l = lines.size()` (one past the last valid index into `lines`), but `operator++()` reaching end keeps `l = lines.size() - 1`. This inconsistency causes `operator--()` and `operator+=(-n)` to malfunction on multi-line FViews when called on the `end()` iterator: the first `--l` goes from `lines.size()` to `lines.size()-1` (the same last line), double-counting the last line's elements instead of retreating to the previous line.
    - **Fix:** Change `end()` to store `l = lines.size() - 1` (the last valid line index) while keeping `i = i1` as the one-past-end position. Equality/ordering comparisons use `local_off`, so this is safe. Verify that `operator++` at the last element still correctly produces the end state (it already avoids the if-branch when `l == lines.size() - 1`).
    - **Test:** Add a multi-line FView test case to `selections.t.cpp` with at least 2 lines. Verify: (a) forward iteration produces correct values across line boundaries, (b) `operator--` from `end()` produces the last element, (c) `operator+=(-n)` from `end()` correctly crosses line boundaries.
    - File: `src/mesh/selections.hpp` (`end()` method, ~line 575), `src/mesh/selections.t.cpp`.
...
- [x] **7.H1** Fix `zip_transform_view::begin() const` / `end() const` iterator type mismatch (`src/fields/lazy_views.hpp`): **DONE** — added `using const_iterator = detail::zip_transform_iterator<F, const Rngs...>` and updated `begin() const`/`end() const` to return `const_iterator`.
  - File: `src/fields/lazy_views.hpp`.

- [x] **7.H2** Fix `multi_slice_view::iterator::operator++` unconditional `operator+=` on base iterator (`src/fields/selector.hpp`): **DONE** — replaced `base_it_ += n` and `base_it_ -= n` with `std::ranges::advance(base_it_, n)` in `operator++()` and `operator--()`.
  - File: `src/fields/selector.hpp`.

- [x] **7.H3** Add `.base()` accessors to `multi_slice_view`, `predicate_view`, and their iterators (`src/fields/selector.hpp`): **DONE** — added `base()` to `multi_slice_view` (view + iterator) and `predicate_view::iterator`. `predicate_view` already had `base()`.
  - File: `src/fields/selector.hpp`.

- [x] **7.H4** Add `#include <fmt/ranges.h>` to `heat.cpp` for `fmt::join` (`src/systems/heat.cpp:167`): **DONE**.
  - File: `src/systems/heat.cpp`.

- [x] **7.H5** Fix `optional_view::begin()` ternary type mismatch when `iterator_t != sentinel_t` (`src/fields/selector.hpp`): **DONE** — replaced `keep_bounds_ ? begin(base_) : end(base_)` ternary with `auto it = begin(base_); if (!keep_bounds_) advance(it, end(base_)); return it;` to avoid type mismatch when the base range uses different iterator/sentinel types (e.g., `counted_iterator` vs `default_sentinel_t` from `std::views::take`).
  - File: `src/fields/selector.hpp`.

- [x] **7.H6** Fix `plane_view<1>::iterator` unconditional `operator+=`/`-=` on base iterator (`src/fields/selector.hpp`): **DONE** — replaced all `base_it_ += n` and `base_it_ -= n` in `operator++()`, `operator--()`, and `operator+=(n)` with `std::ranges::advance(base_it_, n)`.
  - File: `src/fields/selector.hpp`.

- Test: `cmake --build build --target src/systems/CMakeFiles/shoccs-system.dir/heat.cpp.o` compiles successfully. t-heat still blocked on I/O library migration (7.8-7.10). All previously-passing tests (t-cartesian, t-selections, t-shapes, t-mms) still pass.
...
13. **7.H1–7.H6** (heat.cpp build fixes) are done. heat.cpp compiles successfully.
14. **7.1f** (C++23 guard) is low priority and independent of all other items.
15. **7.H1**–**7.H6** are done (heat.cpp now compiles). Next: **7.8** (field_io.cpp), **7.9** (field_data.cpp), **7.10** (xdmf.cpp), **7.11** (simulation verification), **7.12** (scalar_wave.cpp), **7.13** (stencil tests), **7.14** (mesh.t.cpp).
