...
### D-Opt-2: Multi-Fidelity Bayesian Optimization Architecture (Plan 47)
**Decision:** Four cross-cutting choices for the cost-aware multi-fidelity Bayesian-optimization driver shipped in plan 47:

**(a) BoTorch chosen over Emukit, Trieste, and Ax.**
BoTorch 0.17.x ships clean aarch64 wheels for `botorch`, `torch` (CPU-only via the PyTorch CPU index), and `gpytorch` — `uv sync` resolves without a source-build hack like the nlopt git override plan 46.0 needed. The PyTorch backend keeps NumPy 2 compatibility (Emukit pins NumPy 1.26 via GPy; Trieste pins TensorFlow 2.16 with NumPy 1.26 — both would clash with the rest of the stencil_gen stack). BoTorch has first-class multi-fidelity APIs (`qMultiFidelityKnowledgeGradient`, `MultiTaskGP`, `IndexKernel` for ICM, `AffineFidelityCostModel`/`InverseCostWeightedUtility`) all stable in 0.17.x and matching the discrete-fidelity tutorial 1:1. Active 2026 maintenance: weekly commits, frequent point releases, integrates with Ax 1.0. CPU-only torch wheel is ~140 MB (vs ~3 GB if the default index pulls the NVIDIA CUDA stack); a `[tool.uv]` `extra-index-url = ["https://download.pytorch.org/whl/cpu"]` plus `index-strategy = "unsafe-best-match"` keeps the resolver on the CPU wheel without breaking NumPy 2 resolution.
**Options:**
- **(a) BoTorch (qMFKG + MultiTaskGP/ICM)** ← CHOSEN
- (b) Emukit (`MultiFidelityModel` + `MultiInformationSourceEntropySearch` — NumPy 1.26 pin via GPy)
- (c) Trieste (TensorFlow 2.16 backend — NumPy 1.26 pin)
- (d) Ax 1.0 (higher-level wrapper around BoTorch — extra abstraction without payoff at our scale)
- (e) Pure-PyTorch hand-roll (~600 lines for GP + KG + cost-aware utility — long-term maintenance burden)

**(b) Discrete-fidelity ICM kernel (`MultiTaskGP` + `IndexKernel`) over Kennedy-O'Hagan autoregressive AR1.**
Kennedy-O'Hagan models `f_m(x) = ρ_{m-1} f_{m-1}(x) + δ_m(x)` — a refinement chain where each fidelity is a noisy approximation of the next. This is wrong for our cascade: `scientific_findings.md` finding #1 documents that L3 (1D periodic advection eigenvalue) and L3r (BL §4.2 reflecting-hyperbolic spectrum) test fundamentally different physics — tension closures pass L3 universally but fail L3r at `max_spectral_abscissa ≈ 0.95`. An AR1 chain with a single global `ρ` cannot represent "L3 ↔ L3r are independent". The intrinsic-coregionalization (ICM) kernel parameterizes the cross-task covariance matrix `B = W Wᵀ + diag(κ)` where `W` is rank-`r` (default rank=2) and learned end-to-end via marginal-likelihood maximization. The data reports the actual layer-pair correlations; we do not bake an assumed structure in. `MultiTaskGP` is BoTorch's purpose-built ICM model — `SingleTaskMultiFidelityGP` hardcodes `LinearTruncatedFidelityKernel` (an AR1 variant) and was rejected at 47.2a after `fit_gpytorch_mll` failed with `NotPSDError` on ~70% of small noise-free synthetic datasets vs `MultiTaskGP`'s 20/20 success rate.
**Options:**
- **(a) ICM kernel (`MultiTaskGP` + `IndexKernel`)** ← CHOSEN
- (b) Kennedy-O'Hagan AR1 (`SingleTaskMultiFidelityGP` + `LinearTruncatedFidelityKernel` — wrong for L3↔L3r independence)
- (c) Linear-truncated continuous fidelity (BoTorch's default for `s ∈ [0, 1]` — our cascade has discrete layer indices)

**(c) Cost-aware `qMultiFidelityKnowledgeGradient` (qMFKG) primary with `qMultiFidelityMaxValueEntropy` (MES) as a documented one-line swap.**
qMFKG matches the BoTorch discrete-fidelity tutorial's recipe: it composes the Knowledge-Gradient acquisition (one-step lookahead at the posterior over the optimum) with `InverseCostWeightedUtility` to produce a cost-aware EIG-per-second criterion. The combination is well-tested for our problem shape (1–2 design dimensions, K=2–5 fidelities, budget tens of evals). MES is the natural fallback if KG diagnostics show pathology (Gumbel sampling collapse, all fantasies within 1e-6 — observed in Wu et al. 2020 §5.3 on noisy synthetic problems); the swap is a one-line constructor change at the `build_acquisition` site. Both acquisitions accept the same `cost_aware_utility=` and `project=` kwargs, so the surrounding `optimize_acqf_mixed` plumbing is shared. MES was not chosen as primary because the literature consensus (Wu 2020, Frazier 2018) puts qMFKG ahead on practical convergence speed at small budgets when the cost ratio is well-calibrated. The qMFKG `forward(X)` X-layout is `(b, q + num_fantasies, d_total)` — only the `q` candidate slice carries the cost-aware sampling decision; the `num_fantasies` slice is inner-argmax state. This X-slicing detail surfaced as a real bug in 47.3k.3.1 (the HF-acquisition-bonus mask was averaging over both slices, diluting the bonus by ~65×).
**Options:**
- **(a) qMFKG primary, MES as one-line swap** ← CHOSEN
- (b) MES primary (less proven for cost-aware MF; Gumbel-sampling fragility)
- (c) qMFKG only (no fallback path documented)
- (d) `qMultiFidelityHypervolumeKnowledgeGradient` (multi-objective MF — deferred; 47 is single-objective)

**(d) Constant cost model with floor `c'(m) = max(c(m), 0.05 * c(hf))` over per-layer learned cost models.**
Plan 46 measured per-layer wall times to within ±10% (except L7, which is kernel-dependent — 4.7× spread for tension), so the cost table is reliable enough that learned per-layer cost models would not improve acquisition quality at our budget scale. The constant-table approach also keeps the `BOResult.cost_model` field bytewise-stable across runs, simplifying regression tests and post-hoc analysis. The 5% floor `c'(m) = max(c(m), 0.05 * c(hf))` defends against over-exploitation of the cheapest fidelity — without it, a cost ratio of 100×+ (L1 at 0.076 s vs L7 at 1.434 s, ~19×; L1 vs L7+nn at ~230×) makes qMFKG's cost-weighted utility nearly oblivious to HF picks for many iterations, leaving the basin under-resolved. The floor is enforced inside `apply_cost_floor` (`bo.py:558`) and applied to a fresh copy of the user's `cost_table` so the original is unchanged. Per-layer learned cost models (e.g. a small GP per fidelity over `(x, log(wall_time))`) are a future extension; the constant table with floor is the MVP. The 47.3i adaptive cost floor (`adaptive_hf_floor` kwarg) layers on top of the static floor: when both predicates fire (HF posterior uncertain AND cheap surrogate well-fit), the effective HF cost is dropped further toward `α * cheap_min` to break temporary HF coverage starvation; default off.
**Options:**
- **(a) Constant cost table from plan 46 with 5% floor; per-layer learned cost models deferred** ← CHOSEN
- (b) Per-layer GP cost models (`(x, log(wall_time))` regression — adds K small GPs to fit per iteration)
- (c) Constant cost table with no floor (qMFKG over-exploits cheapest layer at 100× ratios)
- (d) Empirical cost from a rolling per-iteration average (CLI's `--cost-model empirical` stub raises `NotImplementedError` — deferred)

**Why these are cross-cutting:** choice (a) sets the optimization-library surface that plan 48 (Brady-Livescu 1D Euler reproduction) will reuse if it adopts a Bayesian surrogate over the nonlinear blow-up scoring (the BoTorch+MultiTaskGP stack is already verified clean on aarch64 — plan 48 inherits that without re-evaluating). Choice (b) sets a precedent for any future MF-BO work over a cascade with non-refinement fidelities (e.g. plan 48's coarse-grid vs fine-grid 1D Euler step might or might not be a refinement chain — the ICM kernel handles either case; an AR1 commitment now would constrain plan 48's modelling choices later). Choice (c) sets the acquisition surface; the documented MES swap means plan 48 (or any future MF problem) can opt into MES without rewriting the BO loop. Choice (d) sets the cost-model surface; per-layer learned costs and the adaptive floor mechanism (47.3i) are the natural escalation paths if plan 48 hits HF starvation on its own cascade.
**Implementing plan items:** 47.0a (BoTorch + CPU-torch dependency stack), 47.2a (`build_mf_gp` + MultiTaskGP/ICM), 47.2b (`DEFAULT_COST_TABLE` + `apply_cost_floor` + `build_cost_model`), 47.3a (`build_acquisition` + qMFKG + MES fallback documentation), 47.3i (adaptive cost floor), 47.3k.3/47.3k.3.1 (HF-acquisition-bonus + X-slice fix).
