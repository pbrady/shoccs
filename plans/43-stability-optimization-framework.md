...
- [ ] **43.9b-r1** Review follow-up on 43.9b: the `cpp_cutcell_violates_197_288` diagnostic flag is currently recorded *by the test* (`r.extras["cpp_cutcell_violates_197_288"] = bool(r.best_x[1] < 197.0/288.0)` at `tests/test_optimizer.py:1101`), and the subsequent `assert isinstance(r.extras["cpp_cutcell_violates_197_288"], bool)` is tautological — it asserts that the value just set via `bool(...)` is a `bool`. This provides zero coverage and means the flag is *not* present in any persisted optimizer output, so the 43.9b bullet claim that "downstream plan 43.10 wiring can consume the flag" is unfulfillable as stated. Two choices, pick (a):
  - (a) **Persist the flag in the pipeline.** Add a small post-processing step in `run_staged_optimize` (or a helper called from `sweeps/optimize.py` when `kernel == "classical"` and `scheme == "E4"`) that populates `extras["cpp_cutcell_violates_197_288"] = bool(best_x[1] < 197.0/288.0)` on the returned `OptimizeResult`. Then update the test to *observe* the flag instead of setting it: remove the self-assignment, assert `"cpp_cutcell_violates_197_288" in r.extras`, and assert `r.extras["cpp_cutcell_violates_197_288"] == (r.best_x[1] < 197.0/288.0)`. Gate the flag so non-E4-classical kernels don't carry it (keep extras uncluttered).
  - (b) **Drop the flag from the plan/test.** If the L8 197/288 diagnostic is strictly a plan-43.10 concern (computed from `best_x` at validation time), remove both the self-assignment+tautological assertion from `TestStagedClassicalAlpha` and the "downstream plan 43.10 wiring can consume the flag" sentence from the 43.9b bullet. Leaves the test tighter and the plan self-consistent.
  - Either way, the fix should also avoid reinstating flaky behavior — whatever replaces the tautology must be satisfied by the observed seed=0 outcome `α₁ ≈ 0.09 < 197/288` (flag=True) documented in 43.9b.
  - File: `scripts/stencil_gen/tests/test_optimizer.py` (both options); `scripts/stencil_gen/stencil_gen/optimizer.py` or `scripts/stencil_gen/sweeps/optimize.py` (option a only); `plans/43-stability-optimization-framework.md` (option b only, or plan-text cleanup under option a).
  - Test: `cd scripts/stencil_gen && SYMPY_CACHE_SIZE=50000 uv run pytest tests/test_optimizer.py -x -q -k "TestStagedClassicalAlpha" --run-slow` — must still pass in ~2 min at seed=0.
