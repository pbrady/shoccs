...
- [ ] **43.6c** Fix `run_staged_optimize` fallback-path extras asymmetry and `converged` semantics introduced in 43.6a. Two correctness bugs that will bite callers and regression tests as soon as the validator-blowup branch is hit:
  1. The success-path `extras` dict (optimizer.py lines 923–937) sets `inner_best_objective` and `inner_best_x`, but the all-validator-failed fallback (lines 886–898) omits both keys. `test_staged_tension_e4_convergence` at test_optimizer.py:786 already relies on `r.extras["inner_best_objective"]`, so any caller or test that takes the fallback path will `KeyError`. Add both keys (mirroring the success path) to the fallback `extras` dict.
  2. The fallback uses `dataclasses.replace(inner_result, method="staged", ...)` which silently inherits `inner_result.converged=True` even when every validator candidate blew up at the deeper layer — the staged pipeline then advertises `converged=True` with no validator confirmation. Force `converged=False` in the fallback `replace` call (the staged pipeline did not converge at the validator depth, by definition of hitting the fallback).
  - File: `scripts/stencil_gen/stencil_gen/optimizer.py` (lines 872–900).
  - Test: add `test_staged_validator_all_blowups` in `TestStaged`. Monkey-patch `brady2d_stability_score` so the inner stage succeeds but the validator re-run raises every time (or returns a report whose `failed_layer <= inner_gate`). Assert: `r.method == "staged"`, `r.extras["stage"] == "inner"`, `r.converged is False`, and both `r.extras["inner_best_objective"]` / `r.extras["inner_best_x"]` are present and match the inner result. Keep the test fast (<1 s) by mocking the score call — no real L6 runs.
  - Acceptance: `cd scripts/stencil_gen && uv run pytest tests/test_optimizer.py -x -q -k "TestStaged"` — all guards + new fallback test green.

- [ ] **43.6d** Strengthen `test_staged_validator_reorders` so it actually verifies the validator re-ranking behavior the plan's original 43.6b called out ("synthetic test: inner and validator metrics disagree; validator output differs from inner output"). The current assertion `r.extras["stage"] in ("validated", "inner")` accepts either outcome and does not guard against a regression where the validator silently mirrors the inner winner.
  - Replace the test (or add a companion) that drives re-ordering deterministically via monkey-patching — not via tension-E4 L6 runtime, which is slow and data-dependent. Stub `brady2d_stability_score` so:
    - inner stage (max_layer=3) ranks candidates so that x=A scores best on `layer3.max_stab_eig`;
    - validator stage (max_layer=6) ranks candidates so that x=B (distinct from A) scores best on `layer6.transient_growth_bound`.
    Populate the inner `history` with both points (e.g. by making the Sobol seeds land near A and B). Assert `r.extras["stage"] == "validated"` and `np.allclose(r.best_x, B)` and `r.best_x != inner_result.best_x`.
  - Keep the existing tension-E4 integration test as a slow smoke (or drop it if the synthetic test subsumes the L6-path coverage) — the plan spec's requirement is a *deterministic* re-order, which a live integration run cannot guarantee.
  - File: `scripts/stencil_gen/tests/test_optimizer.py`.
