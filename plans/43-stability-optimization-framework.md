...
- [x] **43.10a-r1** Tightened `_run_cpp_validation` PASS criterion so it matches the cascade's L8 verdict (`stable AND final_linf <= L8_FINAL_LINF_TOL`, not `stable` alone). Imported `L8_FINAL_LINF_TOL` from `stencil_gen.brady2d_stability` at the top of `sweeps/optimize.py` and reused it in the banner — no hard-coded `1.0`. The PASS branch now fires only when both predicates hold; the new `stable AND linf > tol` branch emits `WARNING: L8 soft-failure: final_linf=... > L8_FINAL_LINF_TOL=...`; the `stable=False` branch keeps the existing `WARNING: L8 FAIL` text. `best_objective` is still never altered (analytical verdict stands; L8 is diagnostic). Persisted schema unchanged — consumers can re-derive the verdict from `{stable, final_linf}`.
  - Added `TestOptimizeCppValidation` (3 fast tests, ~1.6 s) in `scripts/stencil_gen/tests/test_optimizer.py`. Each test monkeypatches `sweeps.optimize.SHOCCS_BINARY` to a tmp-path file and `sweeps.optimize.brady2d_stability_score` to return a canned `StabilityReport(layer8={...})`. Cases: (i) `stable=True, linf=tol+1.0` → captured stdout contains "soft-failure" and `L8_FINAL_LINF_TOL={tol}`, does **not** contain "L8 PASS"; (ii) `stable=True, linf=tol*0.5` → prints "L8 PASS", no "soft-failure"; (iii) `stable=False, linf=nan` → prints "L8 FAIL", no "soft-failure". Class docstring notes that 43.10b will extend it with a live shoccs-bridge case.
...
- [x] **43.10b** Added `TestOptimizeCppValidation::test_validate_classical_e4_published_alpha` in `scripts/stencil_gen/tests/test_optimizer.py` — a `@pytest.mark.slow` live-pipeline regression that drives `_run_cpp_validation` end-to-end against the real shoccs binary. No `brady2d_stability_score` monkeypatch. Published α sourced from the canonical `sweeps.brady2d_sweep.CLASSICAL_E4_ALPHA` (same source 43.9d uses; `stencil_gen.alpha_extraction` is E2-only, see 43.9d narrative). Inputs: `scheme="E4"`, `kernel="classical"`, `best_params={"alpha": list(CLASSICAL_E4_ALPHA)}`, `best_objective=-1.0`, `N=21`, `t_final=5.0`. Assertions: returned dict non-`None`, `stable is True`, `final_linf <= L8_FINAL_LINF_TOL` (imported from `stencil_gen.brady2d_stability`, not hard-coded), captured stdout contains `"L8 PASS"` and does *not* contain `"soft-failure"` / `"L8 FAIL"`. Skips with `pytest.skip("shoccs binary not built")` when `sweeps.optimize.SHOCCS_BINARY` is absent.
  - File: `scripts/stencil_gen/tests/test_optimizer.py`
  - Test: `cd scripts/stencil_gen && SYMPY_CACHE_SIZE=50000 uv run pytest tests/test_optimizer.py -x -q -k "test_validate_classical_e4_published_alpha" --run-slow` — 1 passed in 20.4 s. Non-slow `TestOptimizeCppValidation`: 3 passed, 1 skipped in 1.5 s. Full non-slow optimizer suite: 94 passed, 6 skipped in 75 s.
