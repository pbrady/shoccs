...
- [ ] **47.4d** Tests in `tests/test_sweep_bo.py` — `TestBOCLI` (6) + `TestBOIO` (4 remaining; `test_load_restores_int_keys`, `test_make_objective_accepts_loaded_report_fields`, and `test_roundtrip_preserves_eval_history` are already in the file from 47.4c.1 — do **not** re-add):
