...
- [x] **40.5c** Persist per-nextra GV optimum in `known_values.json["footprint"]`:
  ...
  - **Carry-over for 40.5d:** the footprint merge test is subtly different from 40.2e/40.3d/40.4e because `footprint` has a **two-level** merge (top-level `kv["footprint"]` dict, then per-entry merge inside it). The regression test must seed **both** a pre-existing entry keyed by a sigma the `--n-sigma 5` run will not reproduce (e.g. `E4_nextra0_tension_3` — the same sigma the real `known_values.json` uses) AND a `_tension_gv` entry with a `preexisting_gv_extra` sentinel, then assert **both** survive the non-GV re-run. A single-sentinel test would miss the two-level bug.
