...
- [ ] **40.3d** Add an automated regression test for `epsilon_sweep.main()` merge pattern (mirrors 40.2e/40.2f for tension):
  - Review pass on commit `b6e4a75` confirmed that 40.3c implements the additive merge pattern in `epsilon_sweep.main()` for both `kv[scheme_key][args.kernel]` and `kv[scheme_key][f"{args.kernel}_gv"]`, but the only verification is the same manual "snapshot → run twice → inspect JSON → restore" workflow that 40.2e was created to retire for tension. Without an automated test, a future refactor of `epsilon_sweep.main()` (or a copy-paste from a non-merging template) can silently re-introduce the exact bug 40.2d fixed for tension. The carry-over note in 40.3c only mentions adding such a test for 40.4c/40.5c, omitting 40.3c itself; but epsilon_sweep handles two kernels and two key namespaces (`{kernel}` and `{kernel}_gv`), so the merge surface is strictly larger than tension's and equally deserving of pinning.
  - Implementation: add `test_epsilon_sweep_main_merges_known_values` in `tests/test_sweep_gv_objectives.py`, modeled directly on `test_tension_sweep_main_merges_known_values`. Seed `kv_path` with `{"E2_1": {"params": {...}, "gaussian": {"epsilon": 1.0, "stable_at": [20,40,80], "gv_error": 1.234, "preexisting_extra_key": "survive"}, "gaussian_gv": {"epsilon": 0.9, "gv_error": 1.234, "stable_at": [20,40], "preexisting_gv_extra": "survive_gv"}}}`. Run `epsilon_sweep.main` twice with `--scheme E2 --kernel gaussian --n-eps 5 --n-values 20`: first without `--include-gv` (assert `gaussian.gv_error == 1.234`, `gaussian.preexisting_extra_key == "survive"`, `gaussian_gv` byte-equal to seed), then with `--include-gv` (assert `gaussian.preexisting_extra_key == "survive"`, `gaussian.gv_error` is finite float, `gaussian_gv["preexisting_gv_extra"] == "survive_gv"`, and `{"epsilon","gv_error","stable_at"} <= set(gaussian_gv)`). Use `monkeypatch.setattr(sweeps_common, "KNOWN_VALUES_PATH", kv_path)` — `epsilon_sweep` reads via `load_known_values()` from `_common`, so monkeypatching only `_common` is sufficient (same as 40.2e). The existing module-local `_seed_kv` is tension-specific; extract it into a parameterized helper (e.g. `_seed_kv_with_keys(path, scheme_key, primary_key, secondary_key)`) or write a sibling `_seed_kv_epsilon` helper — either is acceptable.
  - Mutation check (must be performed and reported in the "Verified" line): temporarily replace lines 279 / 287 of `epsilon_sweep.py` (the `kv[scheme_key][args.kernel] = kernel_entry` and `kv[scheme_key][gv_key] = gv_entry` assignments) with non-merging literals (e.g. `kv[scheme_key][args.kernel] = {"epsilon": ..., "stable_at": ...}`) and verify both branches of the new test fail (one for the `{kernel}` entry losing `preexisting_extra_key` / `gv_error`, one for the `{kernel}_gv` entry losing `preexisting_gv_extra`). Restore.
