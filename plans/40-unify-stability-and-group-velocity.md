...
- [ ] **40.5e** Add an automated regression test for `footprint_sweep.main()` merge pattern (mirrors 40.2e/40.3d/40.4e):
  ...
  - Implementation: `test_footprint_sweep_main_merges_known_values` seeds `kv["footprint"]["E4_nextra0_tension_3"]` with a `preexisting_extra_key` sentinel (matching the real `known_values.json`, where the existing entry is `E4_nextra0_tension_3` with sigma=3.0 — the prior 40.5d draft incorrectly used sigma=6, which is not the real key the smoke run leaves behind, so the merge guarantee was not actually pinned against the real-world data) and `kv["footprint"]["E4_nextra0_tension_gv"]` with a `preexisting_gv_extra` sentinel. Runs `footprint_sweep.main(["--n-sigma","5","--update-known-values"])` (no `--include-gv`) and then with `--include-gv`, asserting both sentinels survive and `gv_error` is refreshed on the second call. Also asserts the 40.5d filter: after the `--include-gv` call, `kv["footprint"]` does not contain `E4_nextra1_tension_gv` (because nextra=1's GV optimum at `--n-sigma 5` is sigma=0, which the new filter rejects).
  - Mutation checks: (1) replace both merge blocks with non-merging literals and confirm each test fails with a `KeyError` on its respective sentinel; (2) revert the 40.5d filter and confirm the new "no `E4_nextra1_tension_gv`" assertion fails.
