...
- [ ] **40.4c** Persist three-way optimum to `known_values.json`:
  - Review pass on commit `5cd827e` confirmed: 40.4b added `best_stable_gv` / `best_stable_gv_sigma` / `best_stable_gv_gamma` to the dict returned by `run_joint_sweep_coarse`, but `run_tension_penalty_sweep` (lines 350–356) does NOT thread any of those keys into its own returned summary. Without that plumbing 40.4c cannot read the value in `main()`. Step 1 below is the missing wiring; steps 2–3 are the persistence work this item already owned.
  - **Step 1 — plumb GV through the summary:** in `run_tension_penalty_sweep`, propagate `coarse["best_stable_gv"]`, `coarse["best_stable_gv_sigma"]`, and `coarse["best_stable_gv_gamma"]` into the returned summary dict under keys `gv_error`, `gv_sigma`, `gv_gamma`. Leave `best_sigma` / `best_gamma` (sourced from `fine`) alone — those remain the eigenvalue-stability optimum. Handle the `coarse["best_stable_gv"] is None` case (no feasible point in the coarse sweep) by passing `None` through.
  - **Step 2 — additive merge in `main()`:** apply the 40.2d/40.3c merge pattern to `kv[scheme_key]["tension_penalty"]`. Read the existing entry with `tp_entry = dict(kv[scheme_key].get("tension_penalty", {}))`, update only the keys this invocation owns (`sigma`, `gamma`, `stable_at`, and conditionally `gv_error` when the coarse sweep produced one), then assign back. This must NOT clobber pre-existing `gv_error`/other keys when the sweep is run a second time, and must NOT overwrite keys a future GV-only invocation might own. Do NOT introduce a `--include-gv` flag for tension-penalty — `eval_point` always computes `gv_error` (40.4a decision), so the only gating is "did the coarse sweep find a stable point with finite GV".
  - **Step 3 — write a new `tension_penalty_gv` top-level key (optional, for parity with 40.2c/40.3c):** if the coarse sweep produced a non-None GV-optimal `(sigma, gamma)`, also write `kv[scheme_key]["tension_penalty_gv"] = {"sigma": ..., "gamma": ..., "gv_error": ...}` using the same additive merge pattern. The cross-check across grid sizes that 40.2c/40.3c used is NOT required here because tension-penalty's coarse sweep is single-grid (n=40 only) — document this in the implementation notes so 40.4d's regression test does not assume `stable_at`.
  - File: `scripts/stencil_gen/sweeps/tension_penalty_sweep.py`
  - Test: `cd scripts/stencil_gen && uv run python -m sweeps tension-penalty --scheme E2 --n-sigma 5 --n-gamma 5 --update-known-values` — the smoke test only checks the happy path; 40.4d adds the automated merge test.

- [ ] **40.4d** Add an automated regression test for `tension_penalty_sweep.main()` merge pattern (mirrors 40.2e/40.2f and 40.3d):
...
40.3a → 40.3b → 40.3c → 40.3d   (epsilon_sweep mirrors tension_sweep — do once tension is verified; 40.3d is an automated regression test for the merge pattern from review of b6e4a75, mirroring 40.2e/40.2f)
  ↓
40.4a → 40.4b → 40.4c → 40.4d   (tension_penalty 3-way objective; 40.4c expanded after review of 5cd827e to spell out the summary plumbing + additive merge pattern; 40.4d adds the merge regression test mirroring 40.2e/40.2f/40.3d)
  ↓
